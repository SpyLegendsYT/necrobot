    ...
    async def youtube(self, ctx, youtuber_id : str = None, channel : discord.TextChannel = None):
        ...
        if youtuber_id is None:
            ...
            def embed_generator(page):
                to_string = [f"[{result[2]}](https://www.youtube.com/channel/{result[2]}): {ctx.guild.get_channel(result[1]).mention} - `{result[4]}`" for result in feeds[page*15:(page+1)*15]]
        ...
        embed = discord.Embed(title=video["title"], description=video["summary"][:2000], url=entry["link"])
