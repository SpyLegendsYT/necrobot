...
class RSS():
    ...
    async def youtube(self, ctx, youtuber_id : str = None, channel : discord.TextChannel = None):
        ...
        It is also possible that the id of the channel be located in the url, a simple check can confirm it, if the end
        of the url (after the last /) is exactly or is similar to the name of the channel then it isn't the id, if it 
        is more or less a random jumble of letters and numbers then it is most likely the id. The bot will tell you
        if it couldn't find a channel to match the id given and will send you a sample video so you can double check it
        ...
        __Example__
        `{pre}youtube UCBcRF18a7Qf58cCRy5xuWwQ #streams` - unbscribe to the youtube channel with this id and send all 
        ...
        if youtuber_id is None:
            ...
            def embed_generator(page):
                to_string = [f"[{result[2]}](https://www.youtube.com/channel/{result[2]}): {ctx.guild.get_channel(result[1]).mention}" for result in feeds[page*15:(page+1)*15]]
        ...
        while not self.bot.is_closed():
            ...
            for feed in feeds:
                ...
                for entry in parsed_feed:
                    if datetime.datetime.strptime(entry["published"], '%Y-%m-%dT%H:%M:%S+00:00') < feed[3]:
                        continue
