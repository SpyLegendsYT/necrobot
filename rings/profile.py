...
class Profile():
    ...
    async def balance_server(self, ctx, index : int = 0):
        ...
        if index < 0:
            return await ctx.send(":negative_squared_cross_mark: | Please pick an index that us more than or equal to 0")

        moneys = [(x, self.bot.user_data[x.id]["money"]) for x in ctx.guild.members if not x.bot]
        moneys.sort(key=lambda x: x[1], reverse=True)

        def _embed_generator(index):
            embed = discord.Embed(title="Money Ranking", colour=discord.Colour(0x277b0), description="Ranking of user's money on the server")
            embed.set_footer(text="Generated by Necrobot", icon_url=self.bot.user.avatar_url_as(format="png", size=128))

            for user in moneys[index*10:(index+1)*10]:
                embed.add_field(name=f"{moneys.index(user)}. {user[0]}", value=user[1], inline=False)
    ...
    async def balance_global(self, ctx, index : int = 0):
        ...
        for x in self.bot.user_data:
            ...
            if not user.bot:
                moneys.append((user, self.bot.user_data[x]["money"]))

        moneys.sort(key=lambda x: x[1], reverse=True)

        def _embed_generator(index):
            embed = discord.Embed(title="Money Ranking", colour=discord.Colour(0x277b0), description="Ranking of user's money throughout Discord")
            embed.set_footer(text="Generated by Necrobot", icon_url=self.bot.user.avatar_url_as(format="png", size=128))
    ...
    async def info(self, ctx, *, user : discord.Member = None):
        ...
        embed.add_field(name="Warning List", value=self.bot.user_data[user.id]["warnings"][ctx.guild.id], inline=False)
