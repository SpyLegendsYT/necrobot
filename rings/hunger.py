...
class Hunger():
    ...
    async def fight(self, ctx, *, tributes):
        tributes_list = [x.strip() for x in tributes.split("|")]
        dead_list = list()
        ...
        await self.bot.add_reaction(msg, "\N{BLACK RIGHT-POINTING TRIANGLE}")
        await self.bot.wait_for_reaction(["\N{BLACK RIGHT-POINTING TRIANGLE}"], message=msg, user=ctx.message.author, timeout=600)
        ...
        await self.bot.say(tributes_list[0] + " is the Winner!")
