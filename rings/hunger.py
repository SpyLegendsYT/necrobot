...
class Hunger():
    ...
    async def fight(self, ctx, *, tributes):
        ...
        async def _phase_parser(event_list):
        ...
        day = 0
        embed = await _phase_parser(self.game_start_events)
        ...
        while len(tributes_list) > 1:
            if day == 6:
                embed = await _phase_parser(self.feast_events)
                msg = await ctx.send(embed=embed)
                await msg.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
                try:
                    await self.bot.wait_for("reaction_add", check=check, timeout=600)
            ...
            if len(tributes_list) >1: 
                embed = await _phase_parser(self.day_events)
                msg = await ctx.send(embed=embed)
                await msg.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
                try:
                    await self.bot.wait_for("reaction_add", check=check, timeout=600)
            ...
            if len(tributes_list) >1: 
                embed = await _phase_parser(self.night_events)
                msg = await ctx.send(embed=embed)
                await msg.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
                try:
                    await self.bot.wait_for("reaction_add", check=check, timeout=600)
