...
class Utilities():
    ...
    async def ping(self, cont):
        ...
        await self.bot.edit_message(pingms, ":white_check_mark: | The ping time is `{.01f} seconds`".format(ping))
    ...
    async def today(self, cont, date : str =""):
        ...
        `{pre}today deaths 02/14` - prints five deaths that happened on the 14th of February"""
    ...
    async def today_deaths(self, date=""):
        ...
        __Example__
        `{pre}today deaths` - prints five deaths that happened today
        `{pre}today deaths 02/14` - prints five deaths that happened on the 14th of February"""
        if date != "":
            date = "/"+date
        ...
        for death in random.sample(res["data"]["Deaths"], 5):
            link_list = "".join(["\n-[{}]({})".format(x["title"], x["link"]) for x in death["links"]])
            embed.add_field(name="Year {}".format(death["year"]), value="{}\n__Links__{}".format(death["text"], link_list), inline=False)
    ...
    async def today_births(self, date=""):
        ...
        __Example__
        `{pre}today births` - prints five births that happened today
        `{pre}today births 02/14` - prints five births that happened on the 14th of February"""
        if date != "":
            date = "/"+date
        ...
        for birth in random.sample(res["data"]["Births"], 5):
            link_list = "".join(["\n-[{}]({})".format(x["title"], x["link"]) for x in birth["links"]])
            embed.add_field(name="Year {}".format(birth["year"]), value="{}\n__Links__{}".format(birth["text"], link_list), inline=False)
