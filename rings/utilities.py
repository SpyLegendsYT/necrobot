...
class Utilities():
    ...
    async def convert(self, ctx, measure : float, symbol, conversion = None):
        ...
        units to imperial and most imperial units to metric.

        {usage}

        __Example__
        `{pre}convert 10 ft m` - convert 10 feet into meters
        `{pre}convert 5km in`  - convert 5 kilometers to inches"""
        def m_to_i(measure, symbol, conversion):
            #step 1: convert measure to lowest value
            index = m_values.index(symbol)
            for value in range(0, index+1):
                measure = measure * 10

            #step 2: convert measure to metric
            measure = measure / 25.4

            #step 3: convert measure to desired conversion
