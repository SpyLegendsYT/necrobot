...
class NecroBotPyDict(PyDictionary):
    ...
    def meaning(term):
        if len(term.split()) > 1:
            ...
            try:
                html = BeautifulSoup(requests.get("http://wordnetweb.princeton.edu/perl/webwn?s={0}".format(term)).text, "html")
...
class Utilities():
    ...
    async def today_events(self, date : str = ""):
    ...
    @commands.command(pass_context = True, aliases = ["elfdict", "elf"])
    async def elfspeak(self, cont, *, word = ""):
        """To help those who interact with Elf and guide them on their way to enlightenment

        {usage}

        __Examples__
        `{pre}elf michmatch` - defines michmatch
        `{pre}elf` - prints all the words in the Elf Speak Dictionary"""
        elfspeak_dict = {
        "michmatch":{
                "type": "Noun",
                "definitions": ["To be incompatible, whether with video games or in romance" , " refers to an inability to match"],
                "examples": ["Oh no, we had a ROTWK michmatch again", "**Person A**: Don't you love me anymore? \n **Person B**: No, our romantic adventure is one big michmatch"]
            }
        }
        
        if word in elfspeak_dict:
            embed = discord.Embed(title="__**{}**__".format(word.title()), url="https://en.oxforddictionaries.com/", colour=discord.Colour(0x277b0), description=elfspeak_dict[word]["type"])
            embed.add_field(name="__Definitions__", value="- " + "\n- ".join(elfspeak_dict[word]["definitions"]))
            embed.add_field(name="__Examples__", value="- " + "\n- ".join(elfspeak_dict[word]["examples"]))

            await self.bot.say(embed=embed)
        elif word == "":
            await self.bot.say("**List of words in the \"Elf Speak Dictionary\"**\n-" + "\n- ".join(elfspeak_dict.keys()))
        else:
