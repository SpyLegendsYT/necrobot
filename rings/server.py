    ...
    async def starboard(self, ctx, member : Union[discord.Member, int] = 1):
        ...
        if isinstance(member, int):
            ...
            def _embed_maker(index):
                ...
                embed.set_footer(text="Generated by NecroBot", icon_url="https://cdn.discordapp.com/avatars/317619283377258497/a491c1fb5395e699148fcfed2ee755cf.jpg?size=128")
                for row in results[index*10:(index+1)*10]:
                    member = ctx.guild.get_member(row[0])
                    embed.add_field(name=str(member) if member else "User left server", value=str(row[1]))  

                return embed

            await react_menu(self.bot, ctx, len(results)//10, _embed_maker, member-1)
        else:
