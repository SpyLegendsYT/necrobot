...
class Server():
    ...
    async def permissions(self, ctx, user : discord.Member = None, level : int = None):
        ...
        if level is None and user is not None:
            level = self.bot.user_data[user.id]['perms'][ctx.guild.id]
            return await ctx.send(f"**{user.display_name}** is level **{level}** ({self.bot.perms_name[level]})")

        if level is None and user is None:
            members = [member for member in ctx.guild.members if self.bot.user_data[member.id]["perms"][ctx.guild.id] > 0]
            members.sort(key=lambda e: self.bot.user_data[e.id]["perms"][ctx.guild.id], reverse=True)

            def _embed_maker(index):
                string = ""
                for member in members[index*10:(index+1)*10]:
                    level = self.bot.user_data[member.id]['perms'][ctx.guild.id]
                    string += f"\n -**{member.display_name}**: {level} ({self.bot.perms_name[level]})"
            ...
            return await react_menu(ctx, len(members)//10, _embed_maker)

        if level < 0 or level > 7:
            return await ctx.send(":negative_squared_cross_mark: | You cannot promote the user any higher/lower")
    ...
    async def automod(self, ctx, *mentions : Union[discord.Member, discord.TextChannel, discord.Role]):
        ...
        for x in mentions:
            if x.id not in self.bot.server_data[ctx.guild.id]["ignore-automod"]:
                ...
                await ctx.send(f":white_check_mark: | **{x.name}** will be ignored by the bot's automoderation.")
                ...
                await ctx.send(f":white_check_mark: | **{x.name}** will no longer be ignored by the bot's automoderation.")
