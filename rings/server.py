...
class Server():
    ...
    @settings.command(pass_context = True, name="broadcast")
    async def settings_broadcast(self, cont, channel:discord.Channel = "", *, message = ""):
        ...
        if discord.Channel == "" or message == "":
            ...
            serverData[cont.message.server.id]["broadcast"] = ""
            serverData[cont.message.server.id]["broadcast-channel"] = ""
            return
        else:
            serverData[cont.message.server.id]["broadcast"] = message
            serverData[cont.message.server.id]["broadcast-channel"] = channel.id
    ...
    @settings.command(pass_context = True, name="starboard")
    async def settings_starboard(self, cont, channel : discord.Channel="", stars : int = 5):
        if channel == "":
            await self.bot.say(":white_check_mark: | **Starboard disabled**")
            serverData[cont.message.server.id]["starboard"] = ""
            serverData[cont.message.server.id]["starboard-count"] = ""
            return

        serverData[cont.message.server.id]["starboard"] = channel.id
        serverData[cont.message.server.id]["starboard-count"] = stars
        await self.bot.say(":white_check_mark: | **Starboard messages will be posted in {} if they get at least {} stars**".format(channel.mention, stars))
