...
class Server():
    ...
    async def settings(self, ctx):
        ...
        embed.add_field(name="Server language", value=f'`{server["language"]}`')
    ...
    async def language(self, ctx, language : str):
        if language not in ["en", "fr"]:
            raise commands.BadArgument("language must be supported, check language list")
        ...
        await ctx.send(f":white_check_mark: | Your server language is now {language}")

    @language.command()
    async def language_list(self, ctx):
        msg = """__**Supported Languages and their maintainers**__
        - fr - Necro: 
        - en - Necro: 
        - de - xxxxx: 
        """
