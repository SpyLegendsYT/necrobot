...
class NecroEvents():
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            try:
                ...
                print('Bot: Ignoring exception in command {}:'.format(ctx.command), file=sys.stderr)
    ...
    async def on_error(self, event, *args, **kwargs): 
        ...
        if isinstance(event, KeyError):
            if isinstance(event.args[0], int):
                user = self.bot.get_user(event.args[0])
                guild = self.bot.get_guild(event.args[0])
                if guild:
                    if not event.args[0] in self.bot.server_data:
                        self.server_data[event.args[0]] = self.bot._new_server()
                    ...
                    guilds = [x for x in self.bot.guilds if user in x.members]
                    for guild in guilds:
                        await self.bot.default_stats(user, guild)
