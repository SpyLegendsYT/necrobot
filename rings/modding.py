...
class Modding():
    ...
    def __local__check(self, ctx):
        if not ctx.guild:
            raise CheckFailure("Cannot use this command outside a server")
    ...
    async def moddb_game(self, ctx, *, game : str):
        ...
        def _embed_generator(page):
            embed = discord.Embed(title=f"__**{game.name}**__", colour=discord.Colour(0x277b0), url=url, description=game.description)
