...
class Moderation(commands.Cog):
    ...
    async def mute_task(self, ctx, user, role, time):
        ...
        if user.id in self.guild_data[member.guild.id]["mutes"]:
            self.guild_data[member.guild.id]["mutes"].remove(user.id)
    ...
    async def on_member_remove(self, member):
        role = discord.utils.get(member.roles, id=self.bot.guild_data[ctx.guild.id]["mute"])
        if role is None:
            return
            
        self.guild_data[member.guild.id]["mutes"].append(member.id)
    ...
    async def on_member_join(self, member):
        ...
        if member.id in self.guild_data[member.guild.id]["mutes"]:
            await member.add_roles(role)
            self.guild_data[member.guild.id]["mutes"].remove(member.id)
