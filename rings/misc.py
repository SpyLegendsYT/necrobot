...
class Misc():
    ...
    async def fight(self, ctx, *, tributes):
        ...
        async def _phase_parser(event_name):
            ...
            embed = discord.Embed(title=f"__**{self.bot.t(ctx, 'hg-title')}**__", colour=discord.Colour(0x277b0), description=f"{' - '.join(tributes_list)}\n{self.bot.t(ctx, 'hg-next')}")
        ...
        while len(tributes_list) > 1:
            ...
            embed = discord.Embed(title=f"__**{self.bot.t(ctx, 'hg-dead')}**__", description="- " + "\n- ".join(dead_list) if dead_list else self.bot.t(ctx, 'none'))
            embed.set_footer(text=self.bot.t(ctx, "generated-necrobot"), icon_url=self.bot.user.avatar_url_as(format="png", size=128))
            msg = await ctx.send(embed=embed)
            await msg.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
        ...
        embed = discord.Embed(title=self.bot.t(ctx, 'hg-winner-title'), description=self.bot.t(ctx, 'hg-winner-message').format(tributes_list[0]), colour=discord.Colour(0x277b0))
        embed.set_footer(text=self.bot.t(ctx, "generated-necrobot"), icon_url=self.bot.user.avatar_url_as(format="png", size=128))
    ...
    async def ow(self, ctx, username, region="eu", hero=None):
        ...
        def get_a_hero_stat(hero_int):
            ...
            prog = " - ".join(prog_list)
            embed = discord.Embed(title=f"**{username.replace('-', '#')}** {self.bot.t(ctx, 'ow-region')}: {region.upper()}", colour=discord.Colour(0x277b0), description=prog)
            embed.set_footer(text=self.bot.t(ctx, "generated-necrobot"), icon_url=self.bot.user.avatar_url_as(format="png", size=128))
            ...
            for stat in to_find:
                try:
                    title = stat
                    if "most" in stat:
                        title = f"{self.bot.t(ctx, 'most')} {stat.replace('most_', '')}"
                    stats.append(f"{title.replace('_', ' ').title()}: {'{0:.2f}'.format(s['general_stats'][stat])}")
                except KeyError:
                    pass

            embed.add_field(name=self.bot.t(ctx, 'ow-general'), value="-" + "\n-".join(stats), inline=False)
            ...
            for stat in s["hero_stats"]:
                ...
                stats.append(f"{title.replace('_', ' ').title()}: {'{0:.2f}'.format(s['hero_stats'][stat])}")

            embed.add_field(name=self.bot.t(ctx, 'ow-hero'), value="-" + "\n-".join(stats), inline=False)
        ...
        async with self.bot.session.get('http://aws.random.cat/meow') as r:
            try:
                ...
                if self.bot.cat_cache:
                    await ctx.send(self.bot.t(ctx, 'cat-overload'), embed=discord.Embed(colour=discord.Colour(0x277b0)).set_image(url=random.choice(self.bot.cat_cache)))
                else:
                    await ctx.send(f":negative_squared_cross_mark: | {self.bot.t(ctx, 'cat-empty')}")
