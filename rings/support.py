...
class Support():
    ...
    @commands.command()
    async def report(self, ctx, command, *, message):
        ...
        `settings`"""
        msg = ":warning:  __{}__ :warning: \n`{}` - {}\nOn {}({})"
        await self.feed.send(msg.format(ctx.author.mention, command, message, ctx.guild, ctx.guild.id))

    @commands.command()
    async def suggest(self, ctx, *, message):
        """Suggest a feature, please be explicit, it is suggested to join the support server to make suggestions so they can be discussed. Use 
        `n!about` to get the server link.

        {usage}

        __Examples__
        `{pre}suggest add reaction menu to riddles` - send a suggestion
        `{pre}suggest make code less bad` - sends a suggest (I try ok)"""

        msg = ":bulb: __{}__ :bulb: \n ```{}``` \nOn {}({})"
        await self.feed.send(msg.format(ctx.author.mention, message, ctx.guild, ctx.guild.id))
