...
class Tags():
    ...
    @has_perms(0)
    async def tag(self, ctx, tag : str, *tag_args = None):
        ...
        if tag in server_data[ctx.message.server.id]["tags"]:
            tag_content = server_data[ctx.message.server.id]["tags"][tag]["content"]
            server_data[ctx.message.server.id]["tags"][tag]["counter"] += 1
            await ctx.channel.send(tag_content.format(server=ctx.message.server, member=ctx.message.author, channel=ctx.message.channel, content=ctx.message.content,**argDict))
        else:
            await ctx.channel.send(":negative_squared_cross_mark: | This tag doesn't exist on this server.")
    ...
    async def tag_create(self, ctx, tag, *, content):
        ...
        """
        tag = tag.lower()
        if tag not in server_data[ctx.message.server.id]["tags"]:
            server_data[ctx.message.server.id]["tags"][tag] = {'content':content.replace("'", "\'").replace('"', '\"'),'owner':ctx.message.author.id, 'created':d.datetime.today().strftime("%d - %B - %Y %H:%M"), 'counter':0}
    ...
    async def tag_del(self, ctx, tag):
        ...
        tag = tag.lower()
        if tag in server_data[ctx.message.server.id]["tags"]:
            if ctx.message.author.id == server_data[ctx.message.server.id]["tags"][tag]["owner"] or user_data[ctx.message.author.id]["perms"][ctx.message.server.id] >= 4:
                del server_data[ctx.message.server.id]["tags"][tag]
    ...
    async def tag_edit(self, ctx, tag, *, text):
        ...
        if tag in server_data[ctx.message.server.id]["tags"]:
            if ctx.message.author.id == server_data[ctx.message.server.id]["tags"][tag]["owner"] or user_data[ctx.message.author.id]["perms"][ctx.message.server.id] >= 4:
                server_data[ctx.message.server.id]["tags"][tag]["content"] = text.replace("'", "\'").replace('"', '\"')
    ...
    async def tag_raw(self, ctx, tag):
        ...
        tag = tag.lower()
        if tag in server_data[ctx.message.server.id]["tags"]:
            await ctx.channel.send(":notebook: | Source code for " + tag + ": ```" + server_data[ctx.message.server.id]["tags"][tag]["content"] + "```")
    ...
    async def tag_list(self, ctx):
        ...
        {usage}"""
        await ctx.channel.send("Tags in " + ctx.message.server.name + ": ```" + ", ".join(server_data[ctx.message.server.id]["tags"].keys()) + "```")
    ...
    async def tag_info(self, ctx, tag):
        ...
        tag = tag.lower()
        tag_dict = server_data[ctx.message.server.id]["tags"][tag]
        embed = discord.Embed(title="__**" + tag + "**__", colour=discord.Colour(0x277b0), description="Created on " + tag_dict["created"])
        embed.add_field(name="Owner", value=ctx.message.server.get_member(tag_dict["owner"]).mention)
