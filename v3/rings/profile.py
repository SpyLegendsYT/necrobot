...
class Profile(commands.Cog):
    ...
    async def info(self, ctx, *, user : discord.Member = None):
        ...
        embed.add_field(name="Permission Level", value=await self.bot.db.get_permission(user.id, ctx.guild.id))

        embed.add_field(name="User Name", value=str(user))
        embed.add_field(name="Top Role", value=user.top_role.name, inline=True)
        
        badges = await self.bot.db.get_badges(user.id)
        embed.add_field(name="Badges", value=' - '.join([x['name'] for x in badges]))
    ...
    async def badges_place(self, ctx, spot : range_check(1, 8), badge : BadgeConverter = None):
        ...
        if badge is None:
            ...
            badge = self.bot.db.get_badge(ctx.author.id, badge["name"])
            if not badge:
    ...
    async def badges_buy(self, ctx, badge : BadgeConverter, spot : range_check(1, 8) = 0):
        ...
        __Examples__
        `{pre}badges buy edain` - buy the edain badge, price will appear once you run the command"""
        has_badge = self.bot.db.get_badge(ctx.author.id, badge["name"])
        if has_badge:
        ...
        await self.bot.db.update_money(ctx.author.id, add=-badge["price"])
