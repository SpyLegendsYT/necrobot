...
def get_users():
    ...
    for user in raw_user_data:
        ...
        print(user_query.format(int(user), users["money"], users["exp"], users["daily"], ",".join(users["badges"]), users["title"]))
...
def get_guilds():
    ...
    for guild in raw_server_data:
        if guild != "starred-messages":
            ...
            print(guild_query.format(int(guild), guilds["mute"], guilds["automod"], guilds["welcome-channel"], guilds["welcome"].replace("'", "''"), guilds["goodbye"].replace("'", "''"), guilds["prefix"].replace("'", "''"), guilds["broadcast-channel"], guilds["broadcast"].replace("'", "''"), guilds["broadcast-time"], guilds["starboard-channel"], guilds["starboard-limit"], guilds["auto-role"]))
    ...
    for guild in raw_server_data:
            if guild != "starred-messages":
                for command in raw_server_data[guild]["disabled"]:
                    print(disabled_query.format(int(guild), command))
    ...
    for guild in raw_server_data:
            if guild != "starred-messages":
                for id in raw_server_data[guild]["ignore-automod"]:
                    print(automod_query.format(int(guild), id))
    ...
    for guild in raw_server_data:
            if guild != "starred-messages":
                for id in raw_server_data[guild]["ignore-command"]:
                    print(command_query.format(int(guild), id))
    ...
    for guild in raw_server_data:
            if guild != "starred-messages":
                for id in raw_server_data[guild]["self-roles"]:
                    print(self_query.format(int(guild), id))
    ...
    for guild in raw_server_data:
        if guild != "starred-messages":
            for tag in raw_server_data[guild]["tags"]:
                ...
                print(tags_query.format(int(guild), tag, tags["content"].replace("'", "''"), tags["owner"], tags["counter"], tags["created"]))

def get_perms():
    perms_query = """INSERT INTO necrobot.Permissions VALUES ({}, {}, {});"""
    for user in raw_user_data:
        for guild in raw_user_data[user]["perms"]:
            print(perms_query.format(int(guild), int(user), raw_user_data[user]["perms"][guild]))
...
def get_badges():
    ...
    for user in raw_user_data:
        for place in raw_user_data[user]["places"]:
            print(badge_query.format(int(user), place,raw_user_data[user]["places"][place] ))
...
def get_gifts():
    ...
    for user in raw_user_data:
        for guild in raw_user_data[user]["waifu"]:
            for gift in raw_user_data[user]["waifu"][guild]["gifts"]:
                print(gift_query.format(int(user), guild, gift, raw_user_data[user]["waifu"][guild]["gifts"][gift]))
    ...
    for user in raw_user_data:
        for guild in raw_user_data[user]["waifu"]:
            for waifu in raw_user_data[user]["waifu"][guild]["waifus"]:
                print(waifus_query.format(int(user), int(guild), waifu))
...
def get_waifu():
    ...
    for user in raw_user_data:
        for guild in raw_user_data[user]["waifu"]:
            ...
            print(waifu_query.format(int(user), int(guild), waifu["waifu-value"], waifu["waifu-claimer"], waifu["affinity"], waifu["heart-changes"], waifu["divorces"], waifu["flowers"]))
