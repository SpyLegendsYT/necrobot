...
SELECT badges.user_id, badges.badge, 0
FROM (
    SELECT u.user_id, unnest(string_to_array(u.badges, ',')) as badge
...
SELECT guild_id, name, content, owner_id, uses, to_timestamp(created_at, 'DD - Month - YYYY HH24:MI')
...
SELECT warn_id, user_id, issuer_id, guild_id, warning_content, to_timestamp(date_issued, 'YYYY-MM-DD HH24:MI:SS.')
